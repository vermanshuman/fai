    ...
    for (int i = 0; i < ordini.size(); i++) {
      ...
      for (int j = 0; j < riga.length; j++) {
        ...
        if (isCoupon == false && "".equals(riga[j].getCodiceMinSan()) &&  "".equals(riga[j].getCodiceEan())) {
          return "individua riga dell'Ordine con Id Ordine "+ordine.getIdOrdine()+", per la quale non risulata essere stato determinato né il codice MinSan né il codice EAN corrispondente al Numero Oggetto "+riga[j].getNumeroOggetto(); 
        }
        //
