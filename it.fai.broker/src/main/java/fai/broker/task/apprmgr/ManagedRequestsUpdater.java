...
class ManagedRequestsUpdater {
    ...
    public void process(String operationName, List<ManagedRequest> managedRequests, boolean bestMatchCanBeMoreThanRequested, boolean isMagazzino) throws Exception {
        ...
        if (bestMatchCanBeMoreThanRequested == true && isMagazzino == false)
            throw new IllegalArgumentException("combinazione di parametri non consentita: nel caso del Magazzino una disponibilita di 100 elementi, se ne sono stati richiesti 70, sar� registrata come pari a 70, mentre per i Fornitori l'acquisto di 100 unit� non pu� essere registrato come 70 unit� acquistate");
        for (ManagedRequest mr : managedRequests) {
            ApprovvigionamentoFarmaco quantitaRichiesta = mr.getRequested();
            if (quantitaRichiesta.getOid() == null)
                throw new IllegalStateException("identificativo (OID) per l'Approvvigionamento Farmaci per Gruppo di Ordini (FAI_APPROVVIGIONAMENTO_FARMACO) non assegnato per l'operazione di " + operationName + "; ci� non � ammissibile perch� la quantit� richiesta deve essere gi� registrata in banca dati (e quindi provvista di OID)");
            ApprovvigionamentoFarmaco quantitaDisponibile = mr.getAvailableBestMatch(bestMatchCanBeMoreThanRequested);
            if (quantitaDisponibile.getOid() != null)
                throw new IllegalStateException("identificativo (OID) per l'Approvvigionamento Farmaci per Gruppo di Ordini (FAI_APPROVVIGIONAMENTO_FARMACO) risulta gi� assegnato (" + quantitaDisponibile.getOid() + ") per la risposta all'operazione di " + operationName + "; ci� non � ammissibile perch� la risposta non pu� gi� essere stata registrata in banca dati (e quindi non pu� essere provvista di OID)");
            ...
            //
            String LOG_PREFIX = "quantit� " + quantitaRichiesta.getCodiceDescr() + " richiesta " + quantitaRichiesta.getQuantita() + ", disponibile " + quantitaDisponibile.getQuantita() + " presso " + (isMagazzino ? "Magazzino" : "Fornitore") + ": ";
            logger.info(LOG_PREFIX + "...");
            if (quantitaDisponibile.getQuantita() >= quantitaRichiesta.getQuantita()) {
                // La quantit� disponibile � sovrabbondante rispetto a quella richiesta.
                // Dal momento che il record relativo alla quantit� richiesta sar�
                // aggiornato mantenendo la "quantit� richiesta", la (maggiore)
                // "quantit� disponibile", come informazione, andr� persa.
                // Ovviamente ci� � accettabile se e solo se si tratta di un Magazzino:
                // nel caso di Fornitori, se il Fornitore da disponibilit� per 100 pezzi
                // non � possibile conservare la richiesta di 70, perch� l'ordine
                // sucessivo per 70 verrebbe poi rigettato
                if (isMagazzino == false)
                    throw new IllegalStateException("nel caso dei Fornitori, l'operazione di " + operationName + " avente per oggetto l'Approvvigionamento Farmaci per Gruppo di Ordini (FAI_APPROVVIGIONAMENTO_FARMACO) non pu� accettare una disponibilit� di " + quantitaDisponibile.getQuantita() + " elementi per una richiesta di " + quantitaRichiesta.getQuantita());
                //
                ...
                // la quantit� disponibile - sia QD - � inferiore a quella richiesta - QR -.
                //
                // Qui se cose si complicano.
                //
                // Utilizzando, per brevit�, gli acronimi:
                //
                ...
                //
                // Ad APPROV quantitaRichiesta, gi� presente in banca dati con la fk verso
                ...
                // che avr� tutte le propriet� di quantitaDisponibile, a meno di
                // QUANTITA, cui sar� assegnata la differenza QR - QD
                //
                // Tuttavia, non basta: vanno riviste tutte le associazioni APPR2RIGA
                //
                // Pu� essere, per esempio, che, a fronte di una richiesta di
                // 6 unit� da equidistribuire su 3 Righe di Ordine - 2 per riga -
                // La dispobilit� di 4 elementi porti dalla situazione:
                //
                ...
                //
                // Se, invece, la quantit� disponibile fosse pari a
                // sole 3 unit�, si avrebe:
                //
                ...
                //
                // Nel secondo caso c'� comunque un record APPR2RIGA
                // che cambia APPROV, ma quello referenziante Riga B
                // viene diviso in 2, di cui uno continua a referenziare
                // quantitaSoddisfatta, ma con la quantit� ridotta
                // da 2 a 1 e l'altro quantitaDaRichiedere
                //
                // ---
                //
                // --- organizzazione degli APPR2RIGA per priorit� ---
                //     (qualora gi� non lo fossero)
                //
                ...
                logger.info(LOG_PREFIX + "quantit� soddisfatta " + quantitaSoddisfatta.getQuantita() + ", status " + quantitaSoddisfatta.getStatus().getStatus().getAcronym());
                storeApprovvigionamentoFarmaco(quantitaDaRichiedere, ItemStatus.VALUE_TO_PROCESS, true);
                logger.info(LOG_PREFIX + "quantit� ancora da richiedere " + quantitaDaRichiedere.getQuantita() + ", status " + quantitaDaRichiedere.getStatus().getStatus().getAcronym());
    ...
    private ApprovToRiga[] getSortedByPriority(ApprovToRiga[] approvToRiga) {
        Arrays.sort(approvToRiga, new Comparator<ApprovToRiga>() {
            ...
            public int compare(ApprovToRiga a1, ApprovToRiga a2) {
                ...
                // rappresetare l'et� del record "riga"
