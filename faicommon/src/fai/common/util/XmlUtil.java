...
public class XmlUtil {
	...
	public static void doParse(InputStream is, ContentHandler handler, Boolean addFakeRoot) throws Exception {
		SAXParserFactory spf = SAXParserFactory.newInstance();
		spf.setNamespaceAware(true);
		SAXParser saxParser = spf.newSAXParser();
		XMLReader xmlReader = saxParser.getXMLReader();
		xmlReader.setContentHandler(handler);
		InputSource inputSource = new InputSource(is);
		String xmlString = convertInputStreamToString(is);
		inputSource = new InputSource( new StringReader( "<comifar>" + xmlString + "</comifar>" ) );
		xmlReader.parse(inputSource);
	}
