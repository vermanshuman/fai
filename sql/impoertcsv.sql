...
couponTitle VARCHAR(6);
  ...
  INSERT INTO FAI_ORDINE_IN_COLLECTION 
   (OID, INPUT_RESOURCE, CREATION_TS, OID_STATUS,MAGAZZINO_ACRONYM,BATCH_ID,UNIQUE_ID)
   		VALUES(colloid, 'FAKE.CSV', CURRENT_TIMESTAMP, 1,'UPS',-1,SYS_GUID);
      LOOP
        BEGIN
         ...
         UPDATE FAI_ORDINE_IN SET QUANTITA = totalquantity WHERE OID=ordoid;
    	  ...
    	  IF qta != 'qta' AND numOrdcurval IS NULL THEN
	        ...
	        OID_ORDINEINCOLLECTION) VALUES(ordoid,TO_NUMBER(numOrd), 0, TO_NUMBER(qta),CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, 1, colloid);
...
(OID, NUMERO_OGGETTO, TITOLO_INSERZIONE, ETICHETTA_PERSONALIZZATA, QUANTITA, PREZZO_FINALE, OID_ORDINEIN, CREATION_TS, CODICE_MINSAN, CODICE_EAN, IS_COUPON, MAGAZZINO_ACRONYM,ALIQUOTA_IVA_INCLUSA)
VALUES(ordrigaoid, minsan, couponTitle,  minsan,  TO_NUMBER(qta), 0, ordoid,SYSDATE  , minsan, minsan, coupon, 'UPS',0);
