package fai.broker.supplier.driver.magazzino;
...
public class FakeSupplierService extends AbstractSupplierService {
  ...
  public List<DisponibilitaTemp> getAvailabilityNoCache(List<DisponibilitaTemp> fabbisogno) throws Exception {
    ...
    for (DisponibilitaTemp fabb : fabbisogno) {
      ...
      //
      fai.common.db.SqlQueries.logInfo("inoltro richiesta di disponibilità al "+asShortDescr()+" ...", fabb.toXml(), null, conn);
      conn.commit();
      //
      ...
      //
      fai.common.db.SqlQueries.logInfo("risposta ricevuta per la richiesta di disponibilità inoltrata "+asShortDescr(), fabb.toXml(), null, conn);
      conn.commit();
      //
